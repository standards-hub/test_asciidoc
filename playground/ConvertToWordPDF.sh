...
convertOne() {
	...
	docker run --rm -v "${curPath}":"${curPath}" -w "${curPath}" "${PD_DOCKER_IMG}" --no-highlight -o output/"$2".docx "$1" 

	echo "Converting Word to PDF"
	docker run --rm -it -v "${curPath}":"${curPath}" -w "${curPath}" --name libreoffice-headless "${LO_DOCKER_IMG}" --headless --convert-to pdf:writer_pdf_Export --outdir `pwd`/output "$2".docx
}
